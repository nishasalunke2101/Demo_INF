...
source /home/inferyx/monika/configg.sh
...
updateDiscriptions(){
response=$(get_api_call "$CONF_API_BASE_URL/wiki/rest/api/content/2851076?expand=version" "$AUTH_TOKEN")
pageNumber=$(echo "$response" | grep -o '"number":[0-9]*' | awk -F':' '{print $2}')
echo "pageVersion $pageNumber"
wait
newPageNumber=$((pageNumber + 1))
echo "newPageNumber $newPageNumber"
baseurl="https://monikagog.atlassian.net/wiki/spaces/$SPACE_KEY/pages/$CHILD_PAGE_ID/Release+Notes+$SPRINT_NAME"

# Get existing content
existing_content=$(curl --location --request GET 'https://monikagog.atlassian.net/wiki/rest/api/content/2851076?expand=body.storage' \
--header 'Authorization: Basic ATATT3xFfGF00k1CDA4fGb0GTThPXap8nzLiaWufe8Y-hY4kYHXnWLj7EPIlut6FXYpVYru3iZYD6_jQ02w8Fr2TsTN1wzLJxBNI1U15k_jItobyCu66tVKqWx5dDkdiAyjkfkAJv-gB20V1CHSJEIs51CjmZ74hkdWFBgeR912KKE7QjL7x6IE=987F6BDC' \
